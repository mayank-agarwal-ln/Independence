...
if __name__ == '__main__':
    ...
    tfs = []  #TODO: What is tfs - list of the expected values from the PDF 
    ...
    for i in range(numTP + 1):
        ...
        tp += interval #FIXME: This should not be done here
        tfp = testF(tp)
        tfs.append(tfp)
        tp += interval
    ...
    for t in range(len(traces)):
        ...
        plt.plot(testPoints, fs, label=label)# Plot for each sigma - Calculated PDF
    plt.plot(testPoints, tfs, label='testF(x)', color='#000000', linewidth=3) # Actual PDF Plot
    plt.legend()
    plt.show()
